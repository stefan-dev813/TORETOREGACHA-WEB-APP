...
<head>
    ...
    @vite('resources/css/app.css')
        ...
        <style>
            ...
            <blade media|%20only%20screen%20and%20(max-width%3A%20425px)%20%7B%0D>#fincode-form {
                width: 100% !important;
    ...
    <div class="flex justify-center">
        <div class="md:w-[760px] w-full min-h-screen flex flex-col">
            <div class="fincode-logo pt-2 w-full">
                ...
                <a
                    href="{{ ($is_admin==1)?route('test.user.point'):route('user.point') }}">
                    ...
                    <div class="check-agree">
                        ...
                        <label for="check-agree">
                            ...
                            <a href="{{ route('main.privacy_police') }}"
                                class="ml-2 text-blue-600 underline underline-offset-4">見る</a>
                        </label>
                    </div>
                </div>
                <div class="w-full flex justify-center pay-method">
                    <button onclick="makeCardPay()"
                        class="rounded-lg border bg-white h-11 p-2 w-56 justify-center border-cyan-500 hover:bg-slate-100 flex items-center">
                        <img src="{{ asset('images/icon_card.png') }}" alt="CreditCard"
                            class="h-full" />
                        <span class="ml-2 text-lg font-bold">Card</span>
                    </button>
                </div>
                <div class="flex justify-center w-full pay-method">
                    <button onclick="makePaypay()"
                        class="rounded-lg border bg-white h-11 p-2 w-56 flex justify-center border-rose-500 hover:bg-slate-100">
                        <img src="{{ asset('images/pay_pay_logo.svg') }}" alt="Paypay"
                            class="h-full" />
                    </button>
                </div>
                <div class="flex justify-center w-full pay-method">
                    <button onclick="makeApplepay()"
                        class="rounded-lg border bg-white h-11 w-56 flex justify-center border-green-500 hover:bg-slate-100 items-center">
                        <img src="{{ asset('images/apple-pay-logo.svg') }}" alt="Applepay"
                            class="h-16" />
                    </button>
                </div>
                    ...
                    <div class="card-info card-pay hidden">
                        ...
                        <div class="w-full grid grid-cols-5 gap-1">
                            ...
                            <img alt="International"
                                src="{{ asset('images/credit_cards/5.JPG') }}" class="w-full" />
                        </div>
                    </div>
        ...
        <script>
            ...
            let purchase_process_url =
                '{{ ($is_admin==1)?route('test.user.point.purchase_process'): route('user.point.purchase_process') }}';
            var backUrl =
                '{{ ($is_admin==1)?route('test.user.point'):route('user.point') }}';
            var purchase_successUrl =
                '{{ ($is_admin==1)?route('test.purchase_success'):route('purchase_success') }}';

            var is_busy = false;
            var _token = '{{ csrf_token() }}';

            var ui;
            var card_result, order_id = null,
                access_id = null;
